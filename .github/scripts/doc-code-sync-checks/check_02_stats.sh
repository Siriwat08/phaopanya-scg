...
# Count actual
actual_gs=$(find src -name "*.gs" -type f | wc -l)
actual_funcs=$(grep -hcE "^\s*function\s+[a-zA-Z_\$]" src/**/*.gs 2>/dev/null | paste -sd+ | bc)
actual_lines=$(find src -name "*.gs" -type f -exec cat {} + | grep -v "^\s*$" | wc -l)

echo "  Actual: $actual_gs files / $actual_funcs functions / $actual_lines lines"

# Check README
failures=0
readme_files=$(grep -oP "\|\s*\*\*Total Files\*\*\s*\|\s*(\d+)" README.md | grep -oP '\d+\s*$')
if [[ "$readme_files" != "$actual_gs" ]]; then
  echo "  ❌ README Total Files: $readme_files (expected $actual_gs)"
  failures=$((failures+1))
fi

# Check that no doc claims wrong counts
for doc in README.md BLUEPRINT.md CONTEXT.md "LMDS Supreme Engineer.md"; do
  ...
  stale=$(grep -cE "(^|[^0-9-])(22|23)\s*ไฟล์|22\s*source files|22\s*\.gs files" "$doc" || echo 0)
  if [[ $stale -gt 0 ]]; then
