...
*   **Thai Geo Enrichment**: `getEnrichedGeoData()` ใน `07_PlaceService.gs` ใช้ Tiered approach:
    ...
    *   Tier 2: Regex to Fuzzy Matching
...
09	09_DestinationService.gs	Master	Trinity Intersection: Person+Place+Geo to Destination
...
5	Checkpoint & Resume	Pipeline >1K rows or >2 min to Time Guard + saveCheckpoint_ + auto-resume trigger
...
3.5	NEARBY_PENDING — Tiered Spatial: ≤50m to AutoMerge, 51-79m Yellow, 80-100m Orange, >100m new	per distance band	MEDIUM
...
06_PersonService uses 5 strategies: exact phone to exact name to exact phonetic to fuzzy name to hybrid context (sold_to_name tie-breaker)
...
Copy
Tier 0: M_ALIAS Fast Track  to fastLookupByShipToName()  // O(1) reverse index

Tier 1: resolvePerson() to getDestsByPersonId()

NOT_FOUND to return null (do NOT use unreliable data)
...
M_ALIAS (8 cols)
  ...
  1 master_uuid      FK to M_PERSON.master_uuid OR M_PLACE.master_uuid
...
1.
Copy M_PERSON_ALIAS to M_ALIAS
2.
Copy M_PLACE_ALIAS to M_ALIAS
3.
...
runMatchEngine()  // in 00_App.gs or menu
  ...
  ↓   06_PersonService.resolvePerson      to may call createPerson

  ↓   07_PlaceService.resolvePlace        to 20_ThGeoService.extractGeoFromAddress

  ↓   08_GeoService.resolveGeo            to may call createGeoPoint
  ...
  ↓     AUTO_MATCH  to 11_TransactionService.upsertFactDelivery

  ↓     CREATE_NEW  to create master + upsertFactDelivery

  ↓     NEEDS_REVIEW to 12_ReviewService to Q_REVIEW
...
8. Security Architecture — SEC-001 to SEC-012
...
SEC-007	Reviewer email not masked	maskReviewerEmail_() to s***i@company.com
SEC-008	OAuth scope creep	10 to 6 scopes (Least Privilege)
SEC-009	Cookie regex non-RFC	RFC 6265 compliant
SEC-010	PII masking incomplete	All log paths covered
SEC-011	Sheet protection incomplete	4 to 8 sheets + Q_REVIEW range
...
Copy
L1: RAM cache (_GLOBAL_* in 01_Config)        to fastest, dies with script

        ↓ miss

L2: CacheService (100KB/key, 6h TTL)         to chunked if >100KB

        ↓ miss

L3: Sheet cache (or @customFunction formulas)  to persistent
...
1.
"What is the project / overview / status" to use lmds-architect (this skill)
2.
"Is my code compliant with the 16 laws" to use lmds-code-reviewer
3.
"Find bugs / critical issues / performance" to use lmds-bug-hunter
4.
"Refactor this long function" to use lmds-refactor-advisor
5.
"Ready to deploy / pre-deploy check" to use lmds-predeploy-checker
6.
"Implement match logic / 8 rules / Trinity" to use lmds-match-engine-builder
7.
"GAS-specific concern: timeout, cache, batch" to use lmds-gas-best-practices
8.
"Build / fix CI / deploy pipeline" to use lmds-cicd-pipeline
9.
"Security check / SEC-001 to SEC-012" to use lmds-security-auditor
10.
"Thai name / address / geo normalization" to use lmds-thai-data-helper
...
If user pastes an error message or a specific failure, jump to § 5 — Symptom to Root cause mapping.
...
5. Symptom to Root Cause Mapping
...
lmds-security-auditor — for SEC-001to012 deep audit (P0 items often overlap).
...
Configure at Settings to Secrets and variables to Actions to New repository secret:

Secret	Required	How to get	Used by
CLASPRC	✅ Yes (for deploy)	cat ~/.clasprc.json after clasp login	02, 04
APPS_SCRIPT_ID	✅ Yes (for deploy)	Apps Script Editor to Project Settings to Script ID	02, 04
...
# 3. Project Settings (*) to Script ID
...
2.
deploy — flatten src/ to clasp push to create version to update Web App URL
3.
...
Copy
# Via GitHub UI: Actions to Deploy to Run workflow to version label
...
Set up at Settings to Environments to production to Required reviewers.
...
Auto-bumps patch version (X.Y.Z to X.Y.(Z+1))
...
"Script ID not found"
APPS_SCRIPT_ID wrong. Verify at Apps Script Editor to Project Settings.
...
GitHub Actions enabled: Settings to Actions to General to Allow all actions
...
At Settings to Branches to Branch protection rules to main:
...
At Settings to Environments to production:
...
# 2. Open PR to all 8 CI workflows run
...
# Settings to Actions to Select workflow to Disable
...
# File to Version history to Restore
...
lmds-security-auditor — for the SEC-001to012 review that the CI partially automates.
...
Meaning: 1 function = 1 job, explainable without the word "AND". Max ~50 lines, hard cap 100 (.eslintrc.yml: max-lines-per-function: 300 is a generous upper bound for god functions; new code must stay under 100). If a function reads + writes + matches + persists to split. Use _ suffix for private helpers. Cyclomatic complexity ≤ 30 (target ≤ 15). Max 6 params (target ≤ 4).
...
1.
Read target file's DEPENDENCIES header to load those.
2.
Grep all other files for CALLED BY matching target to load them.
3.
...
Meaning: Invoice numbers in Google Sheets get auto-formatted as scientific notation (e.g. INV2024070100123 to 2.02407E+12). Always call normalizeInvoiceNo() from 14_Utils.gs before comparison, write, or hash.
...
- **L42** hardcoded index to patch shown below
...
≤ 50m to AUTO_MERGE (treat as same place)

51-79m to YELLOW (auto-merge with flag)

80-100m to ORANGE (review)

100m to treat as new
...
Person    = 06_PersonService.resolvePerson
             ...
             ├─ if exact match by phone to return existing

             ├─ if exact match by name to return existing

             ├─ if exact phonetic match to return existing

             ├─ if fuzzy match (score ≥ threshold) to return existing

             └─ else to return null (caller will CREATE_NEW)


Place     = 07_PlaceService.resolvePlace

             ├─ if exact match by name+address to return existing

             ├─ if exact match by postal code to return existing
...
Geo       = 08_GeoService.resolveGeo
             ...
             ├─ if exact match by lat+lng (within 1m) to return existing

             ├─ if nearby match (within GEO_RADIUS_M) to return existing (Rule 3.5)
...
Destination = 09_DestinationService.resolveDestination
             ...
             ├─ if Person + Place + Geo all exist to look for existing Destination with this combo

             ├─ if existing Destination found to return it

             └─ else to return null (caller will CREATE_NEW or this becomes a NEEDS_REVIEW)
Why all 3 are required
A "destination" semantically means "this person at this place at this GPS". If any is missing:


Missing Person to "who" is unclear

Missing Place to "where" is unclear

Missing Geo to can't route the truck
...
M_ALIAS
  ...
  master_uuid (FK to M_PERSON or M_PLACE)
...
2.
createGlobalAlias() in 21_AliasService.gs — admin/migration
Anywhere else (Group 2 code, helpers, web app actions) to forbidden.
...
5.
Future matching: raw name to M_ALIAS lookup to winner (O(1))
Confidence scoring
...
If candidate A's last_sold_to matches current sold_to_name to A wins

Else if candidate B's last_sold_to matches to B wins

Else to keep both for review
...
// 5. Update README's "8 Rules" section to "9 Rules"
...
function findBestGeoByPersonPlace_(shipToName, ctx) {
  ...
  // Tier 1: Person resolve to destinations
...
1.4	Security audit (SEC-001to012)	lmds-security-auditor skill	12/12 PASS
...
2.2	Script Properties set	Apps Script to Project Settings to Script Properties	GEMINI_API_KEY (or empty), LMDS_ADMINS, SCG_COOKIE (if used)
2.3	.clasp.json correct	cat .clasp.json	scriptId matches Apps Script project (~57 chars), rootDir: "src"
2.4	Sheet backup created	File to Make a copy	backup file URL saved in deploy log
2.5	Sheet protection enabled	menu to "🛡️ ป้องกันข้อมูล Sensitive"	8 sheets protected + Q_REVIEW range
...
#	Check	How to verify	Pass criteria
3.1	SYS_LOG monitoring	Apps Script to Executions to filter "Failed"	0 failed in last 24h
3.2	Error alerts enabled	menu to "🔧 ระบบ & ตั้งค่า to * ตั้งค่า API Key" to set Telegram	Telegram bot token + chat ID set
...
3.4	Daily health check scheduled	GitHub Actions tab to 05-scheduled-health.yml	workflow exists + last run green
...
4.4	9 doc-code-sync checks	GitHub Actions tab to 07-doc-code-sync workflow	all 9 green
...
- [ ] `checkSystemIntegrity()` to "✅ System is ready"

- [ ] `runPreflightAudit()` to all green

- [ ] `setupAllSheets()` to no errors

- [ ] WebApp URL loads all 8 pages

- [ ] Sample 20 rows to 16+ land in FACT_DELIVERY
...
1.
Restore sheet from backup (File to Version history, or from the "Make a copy" backup)
2.
...
Origin of the discipline: makeMatchDecision() was 267 lines to split into orchestrator + 8 small rules across 10b_MatchDecision.gs. runTestMatchDryRun_ was 190 lines.
...
LMDS Security Auditor — SEC-001 to SEC-012 Compliance
...
SEC-007	🟡 MEDIUM	Reviewer email not masked	maskReviewerEmail_() to s***i@company.com	Q_REVIEW shows masked email
...
8. SEC-008 — OAuth Scopes (10 to 6, Least Privilege)
...
10. Additional Security Checks (Beyond SEC-001to012)
...
function testThaiNormalization() {
  ...
  cases.forEach(({ input, expected }) => {
    ...
    if (result === expected) {
      ...
      console.log(`❌ "${input}" to "${result}" (expected "${expected}")`);
...
Edge cases to test
1.
Mixed Thai + English: "SCG Cement บริษัท" to how to handle?
2.
...
7.
Mixed case in English part: "Mr. SOMCHAI" to "somchai"
8.
...
Bangkok districts as "amphoe"	Bangkok has "เขต" (khet), not "อำเภอ"	Map เขต to amphoe in the dictionary
...
5. **Default routing hints:**
   - Code change / refactor / architecture question to §1 + §4 + §5
   - Bug investigation / incident / "why is this broken?" to §2 + §4
   - Pipeline / deploy / GitHub Actions / clasp to §3 + §7
   - Performance / quota / timeout / "6 minutes" / CacheService / LockService to §5
   - Match engine / M_ALIAS / Q_REVIEW / 8 rules / Trinity to §6
   - Pre-deploy / pre-merge / go-no-go / "ready to deploy?" to §7
   - Long function / god function / SRP / split this to §8
   - Security audit / PII / SEC-001 / hardcoded secret to §9
   - Thai text / prefix / คุณ / province / postcode / Double Metaphone to §10
   - High-level system / module navigation / how the 35 files fit together to §11
