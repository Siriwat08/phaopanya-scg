...
> เวอร์ชัน: V6.0.064 (DOC-CODE SYNC) | อัปเดตล่าสุด: 2026-07-16
...
3. [โมดูลทั้ง 39 ไฟล์ (38 production + 1 legacy) — ทำอะไรบ้าง](#3-โมดูลทั้ง-22-ไฟล์--ทำอะไรบ้าง)
...
## 3. โมดูลทั้ง 39 ไฟล์ (38 production + 1 legacy) — ทำอะไรบ้าง
...
| **19_Hardening.gs**         | Security + Audit checks + Sheet protection                | `runPreflightAudit()`, `detectDoubleProcessing()`, `applySheetProtection_UI()`                                                                                                          |
| **22_WebApp.gs**            | Dashboard server + API endpoints                          | `doGet()`, `getAppHtml()`, `include_()`                                                                                                                                                 |
| **22b_WebAppViews.gs**      | Web App view data providers                               | `getDashboardData()`, `getQReviewData()`, `getFactDeliveryData()`                                                                                                                       |
| **22c_WebAppActions.gs**    | Web App action handlers                                   | `handleAction()`, `applyDecisionFromWebApp()`                                                                                                                                           |
| **24_PipelineManager.gs**   | Smart scheduling + Telegram alerts + Preflight            | `runPipelinePreflight()`, `scheduleNextRun()`, `sendTelegramAlert()`                                                                                                                    |
| **26_AuditTrailService.gs** | Audit trail (critical-only scope)                         | `logAuditEvent()`, `getAuditTrail()`                                                                                                                                                    |
| **27_RbacService.gs**       | Role-based access control (3 roles)                       | `isAuthorizedUser_()`, `getUserRole()`, `canPerformAction()`                                                                                                                            |
| **28_WebAppActions.gs**     | Web App actions server (mobile menu)                      | `handleMobileAction()`, `getMobileMenuData()`                                                                                                                                           |
| **29_SnapshotTest.gs**      | Snapshot test harness for refactoring safety              | `runSnapshotTest()`, `compareSnapshots()`                                                                                                                                               |
| **99_Legacy.gs**            | Deprecated functions (compatibility layer)                | (compatibility shims)                                                                                                                                                                   |
...
| **10b_MatchDecision.gs**       | Match decision rules (split from 10_MatchEngine)           | (decision rule helpers)                                                                                                                                           |
| **10d_MatchTestHarness.gs**    | Match engine test harness                                  | `runMatchTest()`                                                                                                                                                  |
| **10e_MatchResolvePersist.gs** | Resolve & persist for Q_REVIEW (split from 10_MatchEngine) | `resolveAndPersistMerge_()`                                                                                                                                       |
...
| **11_TransactionService.gs** | FACT_DELIVERY upsert (34-col array)                   | `upsertFactDelivery()`, `findFactRowByInvoice_()`                                                                                           |
| **12_ReviewService.gs**      | Human-in-the-loop management (4 decisions)            | `enqueueReview()`, `applyReviewDecision()`, `applyAllPendingDecisions()`                                                                    |
| **12b_ReviewReprocessor.gs** | Q_REVIEW post-processor (split from 12_ReviewService) | `reprocessReviewQueue()`                                                                                                                    |
...
เมนู **🚚 LMDS V6.0** ปรากฏที่แถบเมนูด้านบนของ Google Sheets:
...
ระบบ LMDS V5.5.004 (ปัจจุบัน V6.0.064) เพิ่มชั้นความปลอดภัย 12 ด้าน (SEC-001→012):
...
3. **Data Minimization (SEC-004, SEC-010)** — ปกปิด PII ใน Log และ Audit Trail, ตั้ง Protected Ranges สำหรับ 8/19 Sensitive Sheets + Q_REVIEW range
4. **OAuth Least Privilege (SEC-008)** — OAuth scopes ลดจาก 10 → 6 (สิทธิ์ตามที่ใช้จริงเท่านั้น)
5. **RFC 6265 Cookie Regex (SEC-009)** — `sanitizeCookie_()` ใช้ RFC 6265 compliant regex
6. **fetchWithRetry\_ Body Truncation (SEC-012)** — Truncate response body ใน log ป้องกัน leak
...
ระบบ LMDS V6.0 ผ่านการตรวจสอบคุณภาพโค้ดตามกฎเหล็ก 16 ข้อ พบว่ามี 5 ข้อที่ควรแก้ไข (SHOULD_FIX) และได้ดำเนินการแก้ไขครบทุกข้อแล้ว ส่งผลให้ Compliance เป็น 16/16 COMPLIANT (รวม Rule 16: Security-First Design)
...
> **เอกสารนี้จัดทำโดย AI Assistant** | เวอร์ชัน: V6.0.064 | อัปเดตล่าสุด: 2026-07-16
