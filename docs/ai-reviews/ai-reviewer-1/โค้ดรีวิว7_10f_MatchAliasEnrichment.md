 ...
 * LMDS V6.0 — Match Engine: Alias Auto-Enrichment (Single Writer for M_ALIAS)
 * ===================================================
 * PURPOSE:
 *   แยกออกมาจาก 10_MatchEngine.gs [Refactor Plan A, ต่อจาก 10b/10d/10e]
 *   เพื่อลดขนาดไฟล์หลักให้เหลือแค่ orchestration (lock/loop/time-guard)
 *   ยังคงเป็น 🟩 Single Writer สำหรับ M_ALIAS เหมือนเดิมทุกประการ — ย้ายไฟล์
 *   เท่านั้น ไม่เปลี่ยน logic แม้แต่บรรทัดเดียว (verbatim extraction)
 *
 * ⚠️ CROSS-FILE STATE COUPLING (สำคัญ — อ่านก่อนแก้ไขไฟล์นี้หรือ 10_MatchEngine.gs):
 *   _ALIAS_ENRICHMENT_CONTEXT ประกาศไว้ที่นี่ (module-level let) แต่ถูก reset
 *   ผ่าน resetAliasEnrichmentContext_() (wrapper, เพิ่มใน Phase 2) ที่เรียกจาก
 *   3 จุดใน runMatchEngine() (10_MatchEngine.gs) ตอน early-return และใน
 *   finally block — ใช้ได้เพราะทุกไฟล์ .gs อยู่ scope เดียวกัน (GAS V8
 *   concatenates ทุกไฟล์เป็น script เดียว) — เปลี่ยนจากแตะตัวแปร raw ตรงๆ
 *   3 จุด มาเป็นเรียกฟังก์ชันเดียว (ตาม pattern releaseScriptLock_ ใน
 *   14_Utils.gs) เพื่อลด coupling surface — ไม่ใช่การแก้บั๊ก เพราะของเดิม
 *   ทำงานถูกต้องอยู่แล้ว
 *   addEntityToEnrichmentContext_() ก็ย้ายมาที่นี่ด้วยเหตุผลเดียวกัน แต่ถูกเรียก
 *   จาก handleCreateNew_() ใน 10_MatchEngine.gs (cross-file call — ปกติ)
 *
 * DEPENDENCIES:
 *   REQUIRES: (Load Order — ไม่สำคัญจริงจัง เพราะ GAS concatenate ทุกไฟล์ก่อน
 *     execute แต่เรียงตามธรรมเนียมเดิมของโปรเจกต์)
 *     - 01_Config.gs, 02_Schema.gs, 14_Utils.gs (core: SCHEMA, *_IDX, normalizeForCompare)
 *     - 06_PersonService.gs, 07_PlaceService.gs (invalidatePersonAliasCache_,
 *       invalidatePlaceAliasCache_, loadAllPersons_/loadAllPlaces_ ผ่าน caller)
 *     - 21_AliasService.gs (loadGlobalAliasesMap_ ใช้ตรวจ dedup)
 *   CALLED BY:
 *     - persistFactRows_() → 10_MatchEngine.gs (เรียก autoEnrichAliasesFromFactBatch_)
 *     - handleCreateNew_() → 10g_MatchRowProcessor.gs [Phase 2] (เรียก addEntityToEnrichmentContext_)
 *   SHEETS ACCESSED:
 *     - SHEET.M_ALIAS         (Write — single writer, เหมือนเดิม)
 *     - SHEET.M_PERSON_ALIAS  (Write)
 *     - SHEET.M_PLACE_ALIAS   (Write)
 *
 * ARCHITECTURE:
...
// ============================================================
// SECTION 1: Module-level enrichment context state
// ============================================================
...
/**
 * resetAliasEnrichmentContext_ — [Phase 2] Wrapper สำหรับ reset module state
 *   เพิ่มขึ้นแทนการแตะ _ALIAS_ENRICHMENT_CONTEXT ตรงๆ จาก 3 จุดใน
 *   runMatchEngine() (10_MatchEngine.gs) — ตาม pattern เดียวกับ
 *   releaseScriptLock_() ใน 14_Utils.gs (wrap raw operation เป็น named
 *   helper) ประโยชน์: ถ้าโครงสร้าง context เปลี่ยนในอนาคต (เช่น ต้อง log
 *   ทุกครั้งที่ reset, หรือเปลี่ยนจาก null เป็น object ว่าง) แก้ที่เดียวพอ
 *   ไม่ต้องไล่หา 3 จุดข้ามไฟล์ — ลด coupling surface, ไม่ใช่การแก้บั๊ก
 */
...
// ============================================================
// SECTION 2: autoEnrichAliasesFromFactBatch_ + helpers (Single Writer M_ALIAS)
// ============================================================
