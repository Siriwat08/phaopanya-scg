...
| ------- | ---------- | --------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 6.0.073 | 2026-07-23 | AUDIT ROUND 6 — SPRINT 0 QUICK WINS (7 fixes)                               | (1) P2-R6-1: CDN `cdn.tailwindcss.com` → `@tailwindcss/browser@4.3.2` with SRI in Unauthorized.html (2) P2-R6-2: 09_DestinationService.gs lock bare → `releaseScriptLock_(lock)` (3) P2-R6-3: AuditTrail appendRow → setValues (consistency) (4) P2-R6-4: Magic number 8 → `SCHEMA[TEST_MATCH_RESULTS].length` + TEST_MATCH_IDX constants (5) P2-R6-5: Audit trail log mask email via `getMaskedEmail_()` (SEC-013) (6) P2-R6-6: getReviewDetail mask reviewer email via `maskEmailSafe_()` (SEC-014) (7) P2-R6-7: showVersionInfo remove hardcoded "542 functions | 25,421 lines" — refer to README/CHANGELOG instead. Plus: Dependabot alert #5 dismissed (@hono/node-server dev-only transitive, not in production runtime) |
